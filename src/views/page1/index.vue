<template>
  <main class="home">
    ...
    <section class="center" role="main">
      <h1 class="title">悬丝为引，鸣雷作判 · 吟霖</h1>
    ...
    >
      <div class="inner container">
        <div class="center">
          <div class="slogan">此间轨迹，丝线共鸣。</div>
          <div class="meta">
            © <span>{{ year }}</span> 吟霖电子设定集 · 制作：霜落天亦
          </div>
        </div>
      </div>
    </footer>
...
const roses: Rose[] = [];
const ROSE_COUNT_DESKTOP = 9;
const ROSE_COUNT_MOBILE = 3;
...
function initRoses(count: number) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    roses.push({
      baseX,
      y: Math.random() * -h,
      size: 108 + Math.random() * 68, // 稍微精简尺寸
...
// ========== 打字机文案 ==========
// 适合吟霖风格的副标题（偏长句，已为打字器准备）
const lines = [
  "曾是维护秩序的巡尉，今是暗巷独行的狩者。",
  "悬于指端的丝线，系着过往荣光与未来回响。",
  "在港藏的薄雾中，她与名为‘小家伙’的沉默同行者。",
  "罚印，是终结罪恶的句点，亦是她命运的注脚。",
  " 拒绝被定义的旋律，在机械与血肉的共鸣中新生。",
  "每一个选择都如丝线牵引，编织成无法回溯的轨迹。",
  " 她所狩捕的，是混沌，亦是自身闪躲的倒影。",
  "于寂静处聆听，那审判的银丝正震颤低鸣。",
];
